...
export async function seedDatabase() {
  ...
  const { data: insertedTypes, error: typesError } = await supabase
    .from('project_types')
    .upsert(projectTypes, { onConflict: 'name' })
    .select();
  ...
  const cityMap = insertedCities?.reduce((acc, city) => {
    acc[city.name] = city.id;
    return acc;
  }, {} as Record<string, string>);

  const typeMap = insertedTypes?.reduce((acc, type) => {
  ...
  const { error: statsError } = await supabase
    .from('statistics')
    .upsert(statistics, { onConflict: 'category,year' });
